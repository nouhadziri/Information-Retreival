...
def create_document_length_vector(doc_index, term_index):
    ...
    for doc in doc_word_map:
        ...
        for term in doc_word_map[doc]:
            squared_sum += doc_word_map[doc][term] * doc_word_map[doc][term]

        doc_index[doc]['length'] = math.sqrt(squared_sum)
...
def save(inverted_index, doc_index, output_file):
    ...
    sqlite_handler = LabeledSQLiteHandler(output_file)
    ...
    try:
        ...
        for doc_id in doc_index:
            sqlite_handler.insert_doc_entry(doc_id, doc_index[doc_id]['class'], doc_index[doc_id]['length'])
...
def print_full(index_file):
    ...
    sqlite_handler = LabeledSQLiteHandler(index_file)
    try:
